...
//  cout<<"communications_menu::communications_menu 1\n";
  ...
  windows_tcp_button_p = new button("Windows Printer (WIP)"); //2020_12_08
  windows_tcp_button_p -> setStyleSheet("button { background-color: yellow }"); //2020_12_08

  if(centre_p->communicate_by_keyboard_cable)
  {
    //if(QFileInfo::exists("/dev/usb2serial"))  //Udev configuration
    if(QFileInfo::exists("/dev/ttyUSB0"))       //missing Udev Rules on current ISO
    {
  ...
  main_layout_p->addWidget(help_button_p, 7, 0);
  main_layout_p->addWidget(message_p, 1, 1, 3, 1);
  main_layout_p->addWidget(ok_button_p, 7, 1);
  main_layout_p->addWidget(windows_tcp_button_p, 6, 0); //2020_12_08
  ...
  connect(windows_tcp_button_p, SIGNAL(clicked()), this, SLOT(windows_tcp_button_clicked())); //2020_12_08
...
void communications_menu::windows_tcp_button_clicked() //2020_12_08
{
  centre_p->add_waiting_screen(43);
  centre_p->screen_done = true;
}
